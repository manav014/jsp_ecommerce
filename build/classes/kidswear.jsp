...
int c=1;
int d=1;
PreparedStatement ps = con.prepareStatement("select * from kidswear");
...
while(rs.next())
{
 ...
 <div class="alert text-light" align="center" role="alert">
   <h3 style="color:hsl(355, 44%, 22%)"><i><b><%=rs.getString(7) %></b></i></h3>
   </div>
   <br>
   <br>
   ...
   <%
	...
	ps = con.prepareStatement("select * from kidswear where category=?");
   ...
   <div class="card mrcard text-dark bg-light mb-3 mx-auto" style="width: 15rem;height:40rem;">
     <img class="card-img-top img-fluid img-thumbnail" src="<%=rs1.getString(2) %>" height="600" >
     <div class="card-body">
       <h5 class="card-title"><%=rs1.getString(3) %></h5>
       <p class="card-text"><%=rs1.getString(6) %></p>
       <div class="container">
   <form action="product.jsp" method="POST">
     <button class="btn btn-dark" type="submit" value="<%=rs1.getString(1) %>" name="productId"> View Item </button>
     </form>
         </div>
     </div>
   </div>
